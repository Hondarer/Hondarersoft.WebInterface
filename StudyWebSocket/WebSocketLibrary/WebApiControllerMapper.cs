...
{
    public class WebApiControllerMapper
    {
        ...
        private string ExecuteController(HttpListenerRequest req, HttpListenerResponse res, string reqBody)
        {
            ...
            if (path.Equals("/cpumodes") && req.HttpMethod.Equals("GET"))
            {
                //return (new ReadUsersController(req, res, reqBody)).Execute();

                byte[] text = JsonSerializer.SerializeToUtf8Bytes(new CpuModes() { new CpuMode() { Hostname = "localhoost" }, new CpuMode() { Hostname = "hostname2" } });

                res.OutputStream.Write(text, 0, text.Length);
            }
            if (path.Equals("/cpumodes/localhost") && req.HttpMethod.Equals("GET"))
            {
                //return (new ReadUsersController(req, res, reqBody)).Execute();

                byte[] text = JsonSerializer.SerializeToUtf8Bytes(new CpuMode() { Hostname = "localhoost" });
